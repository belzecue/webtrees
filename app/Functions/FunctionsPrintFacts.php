...
class FunctionsPrintFacts
{
    ...
    public static function printFact(Fact $fact, GedcomRecord $record): void
    {
        ...
        $element = Factory::gedcomElement()->make($parent::RECORD_TYPE . ':' . $tag);
        ...
        if ($tag === 'MARR' && $type !== '') {
            $marriage_type_element = Factory::gedcomElement()->make('FAM:MARR:TYPE');
            $label = $marriage_type_element->value($type, $tree);
        ...
        foreach ($l2_matches as $l2_match) {
            switch ($l2_match[1]) {
                ...
                default:
                    ...
                    $element = Factory::gedcomElement()->make($subtag);
                    ...
                    foreach ($l3_matches as $l3_match) {
                        ...
                        $element   = Factory::gedcomElement()->make($subsubtag);
                        ...
                        foreach ($l4_matches as $l4_match) {
                            ...
                            $element = Factory::gedcomElement()->make($subsubsubtag);
                            ...
                            foreach ($l5_matches as $l5_match) {
                                ...
                                $element = Factory::gedcomElement()->make($subsubsubsubtag);
        ...
        foreach (array_merge($amatches1, $amatches2) as $amatch) {
            ...
            if ($person && $person->canShowName()) {
                ...
                if (preg_match('/\n[23] RELA (.+)/', $amatch[2], $rmatch)) {
                    ...
                    $label = Factory::gedcomElement()->make($event->record()::RECORD_TYPE . ':ASSO:RELA')->label();
                } else {
                    $label = Factory::gedcomElement()->make($event->record()::RECORD_TYPE . ':ASSO')->label();
                }
    ...
    public static function printSourceStructure(Tree $tree, array $textSOUR): string
    {
        ...
        if ($textSOUR['PAGE'] !== '') {
            $html .= Factory::gedcomElement()->make('SOUR:PAGE')->labelValue($textSOUR['PAGE'], $tree);
        }

        if ($textSOUR['EVEN'] !== '') {
            $html .= Factory::gedcomElement()->make('SOUR:EVEN')->labelValue($textSOUR['EVEN'], $tree);

            if ($textSOUR['ROLE']) {
                $html .= Factory::gedcomElement()->make('SOUR:EVEN:ROLE')->labelValue($textSOUR['ROLE'], $tree);
            }
        }

        if ($textSOUR['DATE'] !== '') {
            $html .= Factory::gedcomElement()->make('SOUR:DATA:DATE')->labelValue($textSOUR['DATE'], $tree);
        }

        foreach ($textSOUR['TEXT'] as $text) {
            $html .= Factory::gedcomElement()->make('SOUR:DATA:TEXT')->labelValue($text, $tree);
        }

        if ($textSOUR['QUAY'] !== '') {
            $html .= Factory::gedcomElement()->make('SOUR:QUAY')->labelValue($textSOUR['QUAY'], $tree);
        }
